...
function makeWord(lettersObject) {
  const result = [];
  Object.entries(lettersObject).forEach(([key, indexes]) => {
...
function sellTickets(queue) {
  ...
  queue.forEach((money) => {
    if (money === TICKET_PRICE) {
      if (TICKET_PRICE in obj) {
        obj[TICKET_PRICE] += 1;
      } else {
        obj[TICKET_PRICE] = 1;
      }
    } else {
      const exchange = money - TICKET_PRICE;
      if (exchange > obj[TICKET_PRICE] * TICKET_PRICE) {
        result = false;
      }
      obj[TICKET_PRICE] -= exchange / TICKET_PRICE;
    }
  });
  return result;
}
